const isObject = (o) => 
    typeof o == 'object' && Object.prototype.toString.call(o) === '[object Object]';

const exists = (o) =>
    typeof o !== 'undefined' && o !== null;

/**
 * Rather than use Exception(s) for flow control, an anti-pattern, as in
 *
 * function getChecklistUrl() {
 *     try {
 *         return dataLayerObject.config.checklist;
 *     } catch (e) {
 *         try {
 *             console.error('dataLayer', e);
 *         } catch (ignore) {}
 *     }
 * }
 *
 * resolvePath allows you to provide an object, and a 'path' to search for.
 * For example, you could reimplement the example function above as
 *
 * function getChecklistUrl() {
 *     return resolvePath(dataLayerObject, '.config.checklist');
 * }
 *
 * By default, if a node doesn't exist along the desired path, resolvePath will return null.
 * You may supply a third argument in this case to specify an alternate default value for a
 * given invocation.
 *
 * resolvePath also lets you set arbitrary values within the object along a given path.
 * If an intermediate node doesn't exist, resolvePath will create them for you in this mode.
 * For the set signature, you must call resolvePath as
 *
 * resolvePath(object, '.some.path', true, data);
 *
 * where data is the value you would like to have set at object.some.path.
 *
 * If you wish to perform a recursive merge at the target path, you may pass true as the last
 * argument in a 'set' invocation.
 *
 * Example usage
 *
 * Fetch path on obj, returning null if path DNE
 * resolvePath(obj, path);
 *
 * Fetch path on obj, returning '' if path DNE
 * resolvePath(obj, path, '');
 *
 * Set value on obj at path
 * resolvePath(obj, path, true, value);
 *
 * Recusively merge data at path on obj
 * resolvePath(obj, path, true, data, true);
 */
const resolvePath = (...args) => {
    var argLength = args.length;

    if(argLength < 2) {
        throw 'resolvePath needs at least 2 args';
    }

    var object       = args[0];
    var path         = args[1];
    var data         = null;
    var defaultValue = null;

    // If there are 4 args, this is a 'set' call
    if(argLength == 4 || argLength == 5) {
        if(args[2] !== true) {
            throw 'The third argument is expected to be true for the set signature of resolvePath';
        }

        data     = args[3];
        var deep = false;
        if(argLength == 5) {
            deep = Boolean(args[4]);
        }
        var m = path.slice(1).split('.');
        var o = object;
        if(typeof o === 'undefined') {
            return o;
        }
        while(m.length) {
            var v = m.shift();
            if(m.length == 0) {
                if(deep) {
                    if(isObject(o[v])) {
                        o[v] = Object.assign({}, o[v], data);
                        break;
                    }
                }
                o[v] = data;
            } else if(typeof o[v] === 'undefined') {
                o[v] = {};
            }
            o = o[v];
        }

        return object;
    } else {
        // Otherwise, this is a 'get' call, in this case we allow an optional third
        // argument, which may specify a default value other than null when a node
        // doesn't exist alonag the desired path.
        if(argLength == 3) {
            defaultValue = args[2];
        }

        // Validation
        if(path.length < 1 || path[0] != '.') {
            throw 'Invalid path ' + path + ' supplied';
        }

        var k = path.slice(1).split('.');

        o = object;

        if (typeof o === 'undefined' || o === null) {
            return defaultValue;
        }

        try {
            var currentKey;

            while (k.length) {
                currentKey = k.shift();

                if (typeof o[currentKey] !== 'undefined' && o[currentKey] !== null) {
                    o = o[currentKey];
                    continue;
                }

                return defaultValue;
            }
        } catch (e) {
            return defaultValue;
        }

        return o;
    }
};

module.exports = {
    exists: exists,
    isObject: isObject,
    resolvePath: resolvePath,
};